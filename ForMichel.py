...
async def register(websocket):
    print("[NETWORK] New WebSocket connection from", str(websocket.remote_address)[1:-1])
    USERS.add(websocket)

async def unregister(websocket):
    print("[NETWORK] WebSocket connection closed for", str(websocket.remote_address)[1:-1])
...
async def main():
    ...
    while True:
        ...
        print("\033[F[CONSOLE] " + line.decode() + "\033[A")
...
if __name__ == "__main__":
    ...
    print("[NETWORK] Running server at http://localhost:%d/" % MYPORT)
